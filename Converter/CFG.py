...
def showDialog(alert_type, message):
    msgBox = QMessageBox()
    msgBox.setIcon(alert_type)
...
class CFG:
    ...
    def loadFrom(self, txtFile):
        """ Costructor From File """
        lines = txtFile
        g = ''.join([re.sub(" |\n|\t", "", x) for x in lines])
        if not re.search('V:' + self._V_set + 'SIGMA:' + self._SIGMA_set + 'S:' + self._S_set + 'P:' + self._P_set, g):
            showDialog(QMessageBox.Critical, 'Error : grammar bad definition, define your grammar as :'
                                             '\nV:[V|V_0],...\nSIGMA:[s|#],...\nS:s0\nP:V1->s1V|#,V2->s1|s2|...')
    ...
    def _parsProductions(self, p):
        ...
        for row in rows:
            ...
            if (left in self._V):
                ...
                showDialog(QMessageBox.Critical, 'Rigth simbol in production ' + row + ' is not in V')
                # raise ImportError('Rigth simbol in production ' + row + ' is not in V')
        if [True] * len(self._V) == [x in self._V for x in v]:
            return P
        else:
            showDialog(QMessageBox.Critical,
                       'Error : not all vocabulary has been used : ' + ''.join([x for x in self._V if x not in v]))
            # raise ImportError(
            # 'Error : not all vocabulary has been used : ' + ''.join([x for x in self._V if x not in v]))
    ...
    def _computeRule(self, rule):
        ...
        while len(_rule) > 0:
            ...
            if r.start() == 0:
                ...
                showDialog(QMessageBox.Critical, 'Error : undefined symbol find in production : ' + _rule)
